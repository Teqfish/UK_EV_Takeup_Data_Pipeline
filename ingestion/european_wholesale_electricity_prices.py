...
def main() -> None:
    ...
    raw_filename = "european_wholesale_electricity_prices.csv"
    prepared_filename = "prepared_european_wholesale_electricity_prices.parquet"
    ...
    # Download source CSV from the web
    print("Downloading electricity prices source file...")
    download_file(source_url, raw_local_path)

    # Read raw CSV
    df_raw = pd.read_csv(raw_local_path)
    ...
    df_prepared["price_eur_mwhe"] = pd.to_numeric(
        df_prepared["price_eur_mwhe"],
        errors="coerce",
    )

    # Keep only required columns and drop bad rows
    df_prepared = (
        df_prepared[["country", "iso3_code", "date", "price_eur_mwhe"]]
        .dropna(subset=["country", "iso3_code", "date"])
        .sort_values(["iso3_code", "date"])
    ...
    print("Electricity prices ingestion complete.")
